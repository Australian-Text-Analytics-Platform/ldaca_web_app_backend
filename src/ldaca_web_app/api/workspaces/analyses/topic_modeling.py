...
async def detach_topic_modeling(
    ...
    if request.topic_meanings_override:
        ...
        # picked — don't re-filter by ``topic_ids`` (the frontend keeps
        # them in sync, but being defensive avoids accidentally dropping
        # rows if a future caller sends only ``topic_meanings_override``).
        filtered_meanings_lf = pl.scan_parquet(override_path).select(
